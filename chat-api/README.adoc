...
[NOTE]
====
If you want to start another instance of mongodb in the same port for example with `docker-compose` then you need to stop this one, so you could run:
...
In order to add support for mongodb database, Nest comes with the ready to use `nestjs-typegoose` package. This is one of the most mature available so far. Since it's written in TypeScript, it works pretty well with the Nest framework.

Firstly, we need to install all of the required dependencies:

[source,sh,options="nowrap"]
----
npm i -s nestjs-typegoose typegoose mongoose @types/mongoose mongodb
----

Once the installation process is completed, we can import the `TypegooseModule` into the root `ApplicationModule`.

[source,ts,options="nowrap"]
----
include::src/app.module.ts[]
----
<1> Import `TypegooseModule` from `nestjs-typegoose` package
<2> Add `TypegooseModule` configurations for mongo database connection
<3> Add models to `TypegooseModule` so they can be injected later in the application components
...
----
<1> Inject typegoose models for: `Message`, `Room`, and `User`.
...
Finally we need to add `messages.controller.ts` containing next code:
